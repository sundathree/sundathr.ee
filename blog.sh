...
# (using templates/post.html). Folders starting with _ are drafts.
set -euo pipefail
shopt -s nullglob
cd "$(dirname "$0")"

marker='<!-- generated by blog.sh'
...
write_list() { # rebuild the post list in index.html from the published posts
    ...
    [ -s "$block" ] || echo "                <li><i>nothing here yet</i></li>" > "$block"

    if ! grep -q '<!-- posts' index.html || ! grep -q '<!-- /posts -->' index.html; then
        rm -f "$rows" "$block"
        die "index.html is missing the <!-- posts --> / <!-- /posts --> markers"
    fi

    awk '
        FNR == NR             { list = list $0 "\n"; next }
        /<!-- \/posts -->/    { printf "%s", list; skip = 0 }
        !skip                 { print }
        /<!-- posts/          { skip = 1 }
